    ...
    for _ in range(count):
        while True:
            ...
            # -- All different chains
                ...
                chain = [base_synset]
                unrelated_syn_1 = get_unrelated_noun(chain)
                chain = chain + [unrelated_syn_1]
                unrelated_syn_2 = get_unrelated_noun(chain)
                chain = chain + [unrelated_syn_2]
            ...
            A, B, C = p.plural(A), p.plural(B), p.plural(C)
            ...
            syllogism = f"{prem1.format(A=A, B=B, C=C)} {prem2.format(A=A, B=B, C=C)} {conclusions[inner_ind].format(A=A, B=B, C=C)}"
...
for i in languages:
        ...
        with open(f"simple/english_simple.json", "w", encoding="utf-8") as f:
                json.dump(all_data, f, indent=4, ensure_ascii=False)

        # ---- Save shared unique nouns ----
        shared_nouns_path = "simple/shared_unique_nouns.json"
