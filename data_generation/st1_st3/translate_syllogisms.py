...
def main():
    # Always use the "simple" folder relative to this script
    base_dir = Path(__file__).parent
    simple_dir = base_dir / "simple_4_var"

    if not simple_dir.is_dir():
        print("'simple' folder not found.")
        return

    input_file = simple_dir / "english_4_var_simple.json"

    if not input_file.exists():
        print("english_simple.json not found inside 'simple' folder.")
        return

    nouns_file = simple_dir / "shared_unique_nouns.json"

    languages = [
        'german', 'spanish', 'french', 'italian', 'dutch',
        'portuguese', 'russian', 'chinese', 'swahili',
        'bengali', 'telugu'
    ]
    ...
    if nouns_file.exists():
        ...
        print("shared_unique_nouns.json not found — running without precomputation.")

    print(f"\nLoading source file: {input_file.name}")

    with open(input_file, 'r', encoding='utf-8') as f:
        data = json.load(f)

    print(f"\nGenerating translations for {len(languages)} languages...\n")

    for lang_index, language in enumerate(languages, 1):
        print(f"[{lang_index}/{len(languages)}] Translating into {language}...")
        ...
        for i, item in enumerate(data, 1):
            ...
            print(f"   [{i}/{total}] done", end="\r")

        output_path = simple_dir / f"{language}_4_var_simple.json"
